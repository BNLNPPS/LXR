...
sub find_incfile {
	my ($incFilename, $pwd) = @_;
        my ($result, $filepath);
        state $FILELISTING = undef;
        state $sourceroot = $config->{'sourceroot'} . '/' . $releaseid . '/';

        if (! $FILELISTING) {
           if ($config->{'swishdir'} && $config->{'swishbin'}) {
	      $FILELISTING = $config->{'swishdir'} . "/$releaseid.filenames"
	   } elsif ($config->{'glimpsedirbase'} && $config->{'glimpsebin'}) {
	      $FILELISTING = $config->{'glimpsedirbase'} . '/lxr/' . $releaseid . "/.glimpse_filenames"
	   } else {
        ...
        if (index($incFilename,'/') == -1 and index($incFilename,'.') == -1) {
           return undef;
        }

        my $filenameOnly = basename($incFilename);
        $filepath = `grep \'/$incFilename\$\' $FILELISTING`;
        if (length($filepath) == 0) {
           $filepath = `grep \'/$filenameOnly\$\' $FILELISTING`;
           if (length($filepath) == 0) {
              return undef;
           }
        }
        ...
        chomp($filepath);
        # $filepath =~ s/^$sourceroot//;

        # yesw
        # print STDOUT "::Lang.pm:: incFilename=$incFilename, filepath=$filepath, pwd=$pwd\n";
        # return undef;

        my $counter = $filepath =~ s/$sourceroot//g;
        if ($counter > 1) {
            $pwd =~ s/^\///;
            $pwd =~ s/\/$//;
            # yesw
            # print STDOUT "::Lang.pm:: filepath=$filepath, pwd=$pwd\n";
            # return undef;
            my @pwd_subdirs = split('/',$pwd);
            my @items = split('\n', $filepath);
            my ($n_mostCommonDir, $item_mostCommonDir, $n_nextMostCommonDir) = (0, '', 0);
            foreach my $item (@items) {
               $item =~ s/^\///s;
               if (dirname($item) eq $pwd) {
                  $item_mostCommonDir = $item;
                  $n_mostCommonDir = 100;
                  last;
               }
               my @item_subdirs = split('/',$item);
               my $n_subdir = 0;
               foreach my $item_subdir (@item_subdirs) {
                  if ($item_subdir eq @pwd_subdirs[$n_subdir]) {
                     ++$n_subdir;
                  } else {
                     last;
                  }
               }
               if ($n_subdir >= $n_mostCommonDir) {
                  $item_mostCommonDir = $item;
                  $n_nextMostCommonDir = $n_mostCommonDir;
                  $n_mostCommonDir = $n_subdir;
               }
            }
            if ($n_mostCommonDir > 0 and $n_mostCommonDir > $n_nextMostCommonDir) {
               $result = $item_mostCommonDir;
            } else {
               return $searchPrefix . $filenameOnly;
               # return undef;
            }
        } else {
          $result = $filepath;
        ...
        #yesw print STDOUT "Testing, result=$result\n";
...
sub _incfindfile {
	...
	push(@paths, $config->incprefix);
        ...
        my $filepath = find_incfile($file, $pwd);
        if ($filepath) {
            #yesw print STDOUT "::Lang.pm:: file=$file, filepath=$filepath\n";
           if ( $searchPrefix eq substr $filepath,0,length($searchPrefix) ) {
              my $filename = substr $filepath, length($searchPrefix);
              my $filename_adv = $filename =~ s/\./\\\./gr;
              $filename_adv = '/' . $filename_adv . '$';
              my $searchLink = "<a class=\"varlink\" href=\""
                                        . $config->{'virtroot'}
                                        . 'search?'
                                        ...
                                        .'_filestring='.$filename_adv ."&_advanced=1"
                                        .'">'.$filename .'</a>';
                                        #, &urlargs ('_filestring=/'.$filename_adv)
                                        #,"\">$filename</a>";
              # print STDOUT "::Lang.pm::searchLink=$searchLink\n";
              return $searchLink;
              #return $filename;
           } else {
              return $filepath;
